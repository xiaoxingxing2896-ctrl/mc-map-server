...
- 2.3.2 修复地图反复缩放与定位体验：图片加载器及内存缓存跨切页复用，视口外保留 256 方块缓冲区，优先请求中心附近瓦片；网络并发上限 3 不再阻塞内存缓存命中。总览使用 256/512 像素解码，放大恢复 1024 原始细节，切换解码尺寸时复用缓存占位，降低大量瓦片造成的内存压力。瓦片直接按视口坐标绘制，避免尺寸约束影响位置。标记/坐标跳转保留当前缩放、立即精确居中，并显示黄色目标准星与坐标；准星独立于标记聚合和隐藏开关。世界坐标使用 Double，修正世界边界附近奇数坐标精度损失，定位针尖端对齐真实坐标。
...
- 视口裁剪与缓冲、最多 3 个并发图片网络请求、按内存比例限制图片缓存、128 MB HTTP 磁盘缓存、公共瓦片索引离线回退。R2 图片版本进入 URL，替换后不复用旧缓存。

...
## 已实现
- 2.3.2：优化瓦片缓存复用和总览内存占用，减少重复加载,跳转保持当前缩放，精确居中并显示黄色目标准星,修正大坐标精度和定位针偏移，已更新相关 MD。

- 2.3.1 优化地图与 Wiki：白边彩色水滴定位针、原创像素分类图案，固定 22×30 dp，密集点按屏幕距离聚合；短按聚合点放大、长按展开列表，同坐标点直接列出，一键隐藏标记。Wiki 默认仅保留紧凑工具栏，分类/收藏等移入菜单，阅读模式隐藏工具栏与应用导航。

- 2.3 完成主题系统第二阶段：本地主题工作室，主题包导入/导出，独立副本编辑、五类场景预览、图片与字体素材、自定义圆角/边框、删除、切换撤回及损坏回退。协议与示例见 [主题工作室](THEMES.md)。

- 2.2 完成主题系统第一阶段：草地方块、幽匿古城、下界堡垒三套内置主题，统一 Atlas 组件与方块导航图标，增加按压、导航、页面和地图跳转动效，以及独立持久化的减少动态/关闭动效和触感设置。详见 [主题与反馈](THEMES.md)。外部主题工作室在 2.3 接入；AI 生成属于第三阶段。

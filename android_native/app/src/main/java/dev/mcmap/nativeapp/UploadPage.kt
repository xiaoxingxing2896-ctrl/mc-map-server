...
@Composable fun UploadPage(vm: AtlasViewModel, back: () -> Unit) {
    ...
    var observedMessage by remember { mutableStateOf(vm.uploadMessage) }
    LaunchedEffect(vm.uploadMessage) {
        if (vm.uploadMessage != observedMessage && vm.uploadMessage?.startsWith("上传成功") == true) feedback?.emit(AtlasFeedback.Success)
        observedMessage = vm.uploadMessage
    }
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Row { AtlasIconButton(onClick = back, enabled = !vm.uploadBusy) { AtlasIcon(Icons.Outlined.ArrowBack, "返回") }; Text("瓦片管理", style = MaterialTheme.typography.headlineMedium) }
        ...
        AtlasOutlinedButton(onClick = { picker.launch(arrayOf("image/png")) }, enabled = !vm.uploadBusy && vm.user?.admin == true, modifier = Modifier.fillMaxWidth()) {
            AtlasIcon(Icons.Outlined.AddPhotoAlternate, null); Spacer(Modifier.width(8.dp)); Text("选择 PNG 瓦片")
        }
        ...
        if (!vm.uploadIndexReady) AtlasTextButton(onClick = { vm.prepareUpload() }, enabled = !vm.uploadBusy) { Text("刷新权限与目标检查") }
        AtlasButton(onClick = { confirm = true }, modifier = Modifier.fillMaxWidth(), enabled = draft != null && vm.uploadIndexReady && !vm.uploadBusy && vm.user?.admin == true) { Text(if (vm.uploadBusy) "上传中…" else if (existing == null) "预览并新增" else "预览并替换") }
    }
    if (confirm && draft != null) AtlasDialog(onDismissRequest = { confirm = false }, title = { Text(if (existing == null) "确认新增瓦片" else "确认替换瓦片") }, text = {
        Text("${worlds[vm.uploadWorld]}\nX ${draft.x} · Z ${draft.z}\n${draft.name}\n\n${if (existing == null) "新增区域将在地图上显示。" else "地图将显示这张新图片。其他管理员若已更新此瓦片，系统会阻止本次替换。"}")
    }, confirmButton = { AtlasTextButton(onClick = { confirm = false; vm.upload(existing != null) }) { Text("确认上传") } }, dismissButton = { AtlasTextButton(onClick = { confirm = false }) { Text("取消") } })
}
